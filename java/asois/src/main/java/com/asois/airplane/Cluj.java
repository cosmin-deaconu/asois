...
//        	Date date = Date.from(dateBeforeXDays.toInstant(ZoneOffset.UTC));
            ...
            for (k=1; k<=rowSize; k++){
    	        ...
    	        status="Arrivals";
    	        st.execute("INSERT INTO cluj VALUES (NULL, '"+dateDB+"','"+time+"','"+flight+"','"+airplane+"','"+aircraft+"','"+status+"')");
    	    ...
    	    String value = null;
            String getDate = null;
    	    String select = "SELECT @date:=date,@count := COUNT(*) from cluj WHERE date like '"+dateDB+"';";
            ...
            String insert="INSERT INTO clujnumber VALUES(NULL,'"+getDate+"','"+value+"');";
            st.execute(insert);
        	Thread.sleep(2000);
